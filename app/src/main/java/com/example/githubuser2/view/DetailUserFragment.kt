...
import com.example.githubuser2.database.DatabaseContract
import com.example.githubuser2.database.DatabaseContract.UserFavoriteColumns.Companion.COLUMN_TYPE
...
import com.example.githubuser2.model.UserDetail
import com.example.githubuser2.view.adapter.SectionsPagerAdapter
import com.example.githubuser2.viewmodel.DetailUserViewModel
...
class DetailUserFragment : Fragment(), View.OnClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        detailUserViewModel.getDetailUser()
            .observe(viewLifecycleOwner, Observer { detailUserItems ->
                if (detailUserItems != null) {
                    showUserDetails(detailUserItems)
                    showLoadingUserDetails(false)
                    fabFavoriteDetailUser.setOnClickListener(this)
                }
            })
    ...
    private fun addUserFavorite() {
        val values = ContentValues().apply {
            ...
            put(
                DatabaseContract.UserFavoriteColumns.COLUMN_NAME,
            ...
            put(
                DatabaseContract.UserFavoriteColumns.COLUMN_COMPANY,
                detailUserViewModel.getDetailUser().value?.company
            )
            put(
                DatabaseContract.UserFavoriteColumns.COLUMN_LOCATION,
    ...
    private fun deleteUserFavorite() {
        userFavoriteHelper = FavoriteHelper.getInstance(requireContext())
    ...
    private fun setFavoriteStatus(state: Boolean) {
        if (state) fabFavoriteDetailUser.setImageResource(R.drawable.ic_baseline_favorite)
        else fabFavoriteDetailUser.setImageResource(R.drawable.ic_baseline_favorite_border_24)
    }
